...
| License | **MIT** (`LICENSE` at the root) | The norm for a library of this kind |
...
4. **Every event is JSON-serializable** and errors travel as message plus typed code.
...
>
> *A deadline on the caller's own context reads as `StopCauseTimeout`, not `StopCauseCanceled`.* The run's `Timeout` cancels with a sentinel cause through `context.WithTimeoutCause`, so the two clocks are told apart; but when the caller's context carries the deadline, the cause really was a clock and `context.Cause` says so. Only an explicit cancel is `Canceled`.
>
> *A run reads the catalog once and checks every turn against it (added 2026-09-15 with the pre-flight).* The lookup happens after the run's context is derived and before the first `TurnStart`; the capability check happens after `BeforeSend` — so it measures what would actually be sent — and before the provider is called, ending a refused run through the same `fail` path as any other error, with the new turn committed first.

> **Added 2026-09-17 while building the nested events.** The ruling above stands; four details of it were settled in code, and the code is the authority.
>
> *The interface is `Reporter`, its method `ExecuteReporting`, and the three values travel in a `ToolOutcome{Result, Usage, Cost}`.* The method cannot also be called `Execute` — an implementation is a `Tool` as well, and one type cannot carry two methods of one name — and the struct rather than three returns is what lets a later fact about a call arrive without editing every implementation's signature.
>
> *`NewReportingTool[In]` is exported beside `NewTool`.* A reporting tool needs the same schema inference, required-member checking and error prose a plain one gets, and the packages that implement the interface — `typesafe` and, in a consumer's own code, anything wrapping a model call — are outside the root package, so the constructor cannot stay unexported. The `Tool` it returns implements `Reporter`; its `Execute` runs the same handler with the reports discarded, for a caller who runs a tool by hand.
>
> *The wire tag is `"tool_event"` and the loop events are eight, not seven.* `ToolEvent{ToolUseID, Name, Event}` is sealed and round-trips at any depth: `decodeEvent` now decodes every event body with the joined event-and-block unmarshalers rather than the block ones alone, which is what lets a `ToolEvent` inside a `ToolEvent` decode through the same table. That dispatch is recursive, so `eventUnmarshalers` is assigned in `init` — a declaration initializer referring to a function that reaches the variable again is an initialization cycle the compiler refuses.
>
> *The reports are funneled on an unbuffered channel the results loop selects on.* Unbuffered is load-bearing: a report is handed over only when the run goroutine takes it, so it is on the stream before the tool that reported it can finish, and the order reported is the order seen. A report is dropped, never waited on, once the consumer has left (a `stopped` channel the loop closes on its way out, before the deferred `wg.Wait`) or once the call has returned (a `returned` channel per call, asked first so a late report is dropped even while the turn's other calls are still reporting). Both are tested, each by a mutation that hangs or leaks without it.
...
**Cache policy.** `Auto` sends the provider's top-level automatic marker plus one explicit breakpoint on the last static system block. `Manual` honors markers the caller sets on blocks and rejects more than four. `Off` sends none. Tools serialize in declaration order.
...
> **Added 2026-09-15 while wiring the catalog into the loop.** The cache is not the agent's, and the facts travel on the request.
>
> *`Request.ModelInfo *ModelInfo` is the seam.* The loop fetches the entry once per run, before the first turn, and puts it on every turn's Request; a direct consumer may fill it in by hand, and nil means "nobody consulted the catalog", never "the model can do nothing". Translation therefore stays a pure function of the Request and a provider's `Stream` never makes a hidden catalog call. Anthropic's thinking translation reads `Capabilities.ThinkingStyle` when it is adaptive or budget and falls back to the model-name heuristic otherwise — with the always-on reading of that heuristic still applied on top of a catalog "adaptive", since the catalog says which *form* a model takes and never that it refuses `disabled`.
>
> *The memo lives on the provider clients, not on `Agent`.* `anthropic.Client` and `openrouter.Client` memoize `Model` per identifier under a mutex, successes only; OpenRouter's single catalog fetch is stored entry by entry, under both id and canonical slug, so the first lookup answers for every model it lists. `Agent` is a plain struct that `chat.NewService` copies by value, so a mutex or an atomic on it would be a `vet` copylocks failure and a split cache. `ModelLister`'s contract says so: the agent calls `Model` at the start of every run, an implementation should memoize, and the built-in providers do. The price is that the first run in a process pays one catalog round trip before its first token — on OpenRouter the whole catalog, a few hundred kilobytes — which a short-lived CLI did not pay before; if that ever bites, the fix is a pre-warm on the client, not a change to the loop.
>
> *The check runs every turn; the fetch runs once.* A tool result can carry an image the caller's first message did not. It requires `CapImageInput` for image blocks, `CapPDFInput` for `application/pdf` documents, `CapTools` for a non-empty tool list, `CapThinking` for an effort other than default or off, and `CapCacheControl` for a caller's marker under `CacheManual`. Tool-use and tool-result blocks in the history ask for nothing of their own: a request replaying tool history always carries the definitions, and charging the blocks would make an image inside a tool result report "tools". Only `Unsupported` refuses (invariant 11); a catalog error, `KindNotFound` included, is logged at Warn and the run proceeds with no facts, because the catalog can lag the API — there is no negative caching in v1. A provider that does not implement `ModelLister` gets no pre-flight at all.
>
> *`Capabilities.MaxOutput` does not override `max_tokens`.* Anthropic refuses a non-streaming request whose `max_tokens` implies more than ten minutes of generation, so sending a model's published 64k or 128k ceiling would break `Complete` on exactly the models that publish the largest one. `anthropic.DefaultMaxTokens` (8192) stands, and the doc comments on `Agent.MaxTokens`, `Request.MaxTokens` and `DefaultMaxTokens` — which promised "the model's maximum" and "a catalog will override it" — are corrected to say that the provider substitutes its own named default.
...
>
> - **`max_tokens` has a named default.** Anthropic requires the member and publishes no default, so `anthropic.DefaultMaxTokens` (8192) is substituted when `Request.MaxTokens` is zero — within every current model's output limit, and to be overridden by `ModelInfo` once the catalog is wired in.
...
> **Added 2026-09-15 while building the Anthropic client, stream, errors and catalog** (`anthropic/client.go`, `stream.go`, `errors.go`, `models.go`):
>
...
> - **`ping` is skipped rather than surfaced.** It is a documented event that says nothing about the message, and an `UnknownEvent` means "goodall did not recognize this" — a keep-alive every few seconds would teach a consumer nothing and train them to filter the very events that matter.
> - **An unrecognized `content_block_delta` type becomes an `UnknownEvent` carrying the whole event's bytes**, not just the delta's, so a consumer has the index and the envelope as well as the payload. `citations_delta` is one of these today; goodall models no citation block yet, so passing it through whole is the honest answer. `BlockStop.Raw` stays empty on this provider: a thinking block is rebuilt exactly from its text and signature.
...
> - **The catalog tree's shape was confirmed against a live `GET /v1/models/{id}`** on 2026-09-15 (`claude-sonnet-4-5-20250929`): `capabilities.thinking` and `capabilities.effort` each carry a branch-level `supported` member *beside* their `types` map and their rungs. `Capabilities.Thinking` reads that branch member first and falls back to the two thinking types, because a model offering a thinking type goodall does not model would otherwise read as unsupported and have the agent refuse a request the model accepts.
> - **`Tools` and `CacheControl` are reported `Supported` without asking the catalog.** Anthropic publishes no leaf for either and every model the Messages API serves takes both, so the alternative is reporting unknown about something that is never in doubt. `Pricing` stays nil: this endpoint publishes no prices, and zero would read as free. `display_name` is decoded and unused — `goodall.ModelInfo` has nowhere to put a human-readable name yet.
...
> **Added 2026-09-15 while recording the OpenRouter fixtures** (`openrouter/testdata/live_*`, `openrouter/fixtures_test.go`, `openrouter/live_test.go`, `scripts/record-fixtures/openrouter.go`). The "hand-authored" bullet above is now half the story: the recorded set sits beside the authored one and the testdata README says which is which. Two members the dialect table did not have, both found by a live call that was refused or silently wrong rather than by reading documentation. `api.openai.com` refuses `max_tokens` on a reasoning model and reads the cap from `max_completion_tokens`, so `Quirks` gained a typed `OutputCap`; and it streams no usage chunk at all unless the request sets `stream_options.include_usage`, so every streamed OpenAI-dialect answer reported zero tokens until `Quirks.StreamUsage` was added (OpenRouter documents the option as a deprecated no-op and always reports usage, so it is not sent there; LM Studio is set to ask too, on the strength of its documentation rather than a live call). One limit of the package is now recorded rather than discovered again: OpenAI's `/v1/chat/completions` refuses function tools alongside any reasoning effort above `"none"` and points at `/v1/responses`, which this package does not speak — so a tool call and reasoning cannot be had in one turn against OpenAI directly. Through OpenRouter they can, because it routes upstream to the Responses API, and that is where the encrypted `reasoning_details` round trip is recorded. A second fact shapes those recordings: an OpenAI model produces its `reasoning_details` on the turn that *answers*, not on the turn that calls the tool, so proving the replay of an entry goodall cannot read takes three turns rather than two. The recorder itself learned to discard a refused request's capture instead of writing its error envelope under a `.sse` name.

## The chat layer

**Threads.** `Thread{ID, Conversation, Usage, Cost, Version, CreatedAt, UpdatedAt}` is JSON-serializable. `ThreadStore` has `Get`, `Put` with a version check, `List` and `Delete`; `MemoryStore` is the built-in implementation and the reference for the contract test any store must pass.
...
> **Corrected 2026-09-15 after the Send-then-Subscribe race was found** (decision leaf tsR4PU). `Send` returns once the run is registered, a finished run's events are not retained, and `Subscribe` on a thread with nothing in flight yields nothing — so the "CLI consumer calls `Send` then `Subscribe` on one goroutine" shape above could see nothing at all when the run finished in between: invisible against a real provider, a coin flip against the fake or a cache hit. Ruled: **`Send` and `Resolve` return a `*Run{ID, Events}`** whose `Events` stream was attached before the run's goroutine exists, so a caller on one goroutine reads it and sees the whole run however fast it ends. It is a subscription like any other — single-consumer, detached by leaving, never able to stop the run — and `Subscribe` keeps its meaning for a second request attaching to a run somebody else started. The alternatives were rejected: retaining each thread's last run (a cache with no eviction across every idle thread) and a run id on `Subscribe` (which only makes the miss detectable; `View` already serves a client that arrives after the answer).
...
>
> *The catch-up is the buffered events themselves, not a synthesized message.* A subscriber attaching mid-run is handed the run's events verbatim and then the live tail, so folding the stream through a `goodall.Accumulator` rebuilds exactly the message an earlier subscriber is looking at. Synthesizing a `MessageStart` plus block events would be a second encoder to keep in step with the first.
>
...
>
> *The run persists before it releases the thread and before it closes its subscriptions,* so a client that reads its subscription to the end and then reads the thread sees the answer it just watched arrive, and `ErrThreadBusy` is returned only while something really is in flight. The write goes out on `context.WithoutCancel` of the service's context, because a stopped or shut-down run reaches persistence with its own context already canceled. A version conflict — someone else wrote the thread meanwhile — is retried once against the thread as it now stands, then logged and given up on.
>
> **Corrected 2026-09-15 after the CLI example was built.** The paragraph above had the order wrong for a client that stops reading at the terminal event. `drive` now persists the thread and frees it *before* the `Done` or `Stopped` is delivered, and closes the subscriptions after: by the time any subscriber holds the terminal event, the thread is in the store and a `Send` on it succeeds. Delivering the event first raced a client that sent its next line at once into `ErrThreadBusy`, intermittently, since it was a race with the store write; the regression test parks the store's `Put` to make the order observable. A client attaching in the window between the run being freed and its subscriptions closing finds no run and reads the thread, which by then holds the answer.
>
...
> **Added 2026-09-15 while building the view and the wire writers** (`chat/view.go`, `chat/wire.go`). What the two paragraphs above leave open, decided in code; the code is the authority.
>
> *The view is computed, never stored.* `NewThreadView(thread, ViewOptions{HasSystemPrompt, ThinkingDisplay})` is a pure function over a thread plus the two facts a thread does not carry, and `Service.View(ctx, threadID)` — with a context, unlike the `View(threadID)` sketched above — reads the thread as `Get` does and calls it. It reflects what is stored: a run in flight is watched through `Subscribe`, not through `View`. The system prompt's *text* is deliberately not a parameter, so a caller computing a view by hand cannot leak it either.
>
> *Everything is a typed struct with a kind, not a map.* `ThreadView{ID, System, Messages, Usage, Cost, Version, CreatedAt, UpdatedAt}`; `MessageView{Role, Content, Partial}`; `BlockView{Kind, Text, Media, ToolUseID, ToolName, Status, BlockType, Placeholder}` where `BlockViewKind` is one of text, image, document, tool_call, tool_result, thinking, redacted_thinking and unknown, and `ToolResultStatus` is `ok` or `error` rather than a bool. Thinking text is carried unless the agent's display is `DisplayOmitted`, in which case the block is still shown — the model did think — with nothing to read. A bytes source on user media becomes a placeholder, because the front end sent the file and has it; URL and file-id sources pass through. Signatures, redacted-thinking payloads and an unknown block's provider bytes have no field at all, so they cannot travel.
>
> *Placeholder ids are digests of position, not of content.* `hex(sha256(threadID + "\x00" + position))[:32]`, where position is the message and block index, or the literal `system` for the prompt. Hashing the secret would make the id a verifier for a guessed value and would let two threads carrying the same tool result advertise the fact by sharing an id. Position is stable because history is append-only, so two computations of one stored thread agree and a front end may key its own state by these ids.
>
> *`WriteSSE(w, events)` and `WriteNDJSON(w, events)` take a `goodall.Stream` — a run stream or a `Subscribe` stream — and write the events' own JSON.* SSE names each frame after `ev.Type()` and writes one `data:` line per newline-separated segment (goodall's JSON is always one line; the split is there so a payload that gained a newline is not silently truncated by a conforming parser). A stream error — `ErrSubscriberOverflow` or anything else — becomes one final frame named "error", carrying `{"type":"error","message":…,"code":…}` with `code` a typed `WireErrorCode` (`subscriber_overflow` or `stream_error`), and the writer returns nil: the failure reached the client. The only error returned is a failure to write, and the stream is unwound on the way out. Frames are flushed through a locally declared `interface{ Flush() }` rather than by importing net/http, so a CLI consumer of the chat layer does not link net/http for one method. There are no `id:` lines and no catch-up marker: nothing in the service marks where a subscription's backlog ends, which remains a known gap.
...
> **Added 2026-09-15 while building the CLI example** (`examples/cli`). The CLI is `main.go` — flags, provider, signal wiring — plus `Loop(ctx, *chat.Service, io.Reader, io.Writer, Options)`, so the preview is testable: five tests drive it against `internal/fake` through an `io.Pipe`, and two log their transcript so `go test -v ./examples/cli/` shows the rendering. It reads the `*chat.Run` that `Send` returns and never calls `Subscribe`. Ctrl-C is one channel with two meanings — during an answer it is `Service.Stop` and the loop stays at the prompt with the partial message kept; at the prompt it ends the session and `Service.Shutdown` drains. Default models are constants in the example, because an example imports only the public packages and so cannot read `internal/livemodel`; they are deliberate copies. It does not use `chat/markdown`, which renders HTML for the web example. Two things the first real consumer noticed: a client that stopped reading at the terminal event raced its next `Send` (corrected in the service, see "The chat layer"), and `TurnEnd` is emitted before the turn's `ToolCallStart` events, since the model call really has ended, so a renderer that wants the usage line after the tool block holds it back itself.

> **Added 2026-09-15 while building the web example** (`examples/web`). What a first HTTP consumer of the chat layer settles, decided in code.
>
> *Six routes and nothing else:* `GET /` (the page, embedded), `POST /threads` → a `ThreadView`, `GET /threads/{id}` → `Service.View`, `POST /threads/{id}/send` → `{"run_id"}` with 409 for `ErrThreadBusy` and 404 for `ErrThreadNotFound`, `GET /threads/{id}/events` → `chat.WriteSSE` over `Subscribe` on the request's context, `POST /threads/{id}/stop` → 204 with 409 for `ErrThreadIdle`. No route marshals a `Thread` or a `Conversation`. The send route discards the `*Run`'s stream — the run outlives the request and the browser attaches through the events route — which is safe because an unread subscriber is dropped once its bounded channel fills.
>
> *The event stream is not redacted, and the view is.* **Superseded 2026-09-15 by `chat.Redact`; see the entry below.** `WriteSSE` over `Subscribe` carries tool inputs (`tool_input_delta`, `tool_call_start`), tool result content (`tool_call_end`) and the entire conversation (`Done.Result.Conversation`), verified against a live Anthropic run. The system prompt is the one thing that cannot travel — `Result` has no field for it — which is what makes the "the page never receives the system prompt" criterion hold. Anything stricter than "the streaming client is trusted with the history" needs a redacting event writer beside `WriteSSE`; that is filed as a leaf under the chat layer, with a second one for the fact that nothing carries the in-flight user message to a second subscriber (the page keeps the pending line in `localStorage` meanwhile).
>
> *The failure frame is named `stream_error`.* A browser's `EventSource` delivers its own connection failures to a listener named `error`, so a frame under that name would land in the same handler; the rename (from the wire writers' first draft) keeps the two apart. The page closes its source itself, because an `EventSource` reconnects for ever to a stream that merely ends, including the empty stream a thread with nothing in flight returns; after a terminal event the run is over, after events without one the connection dropped and is worth one reattach, and with no events at all nothing was running.

> **Added 2026-09-15 while closing the gap the web example found** (`chat/redact.go`). The event stream is redacted too, by a pure mapping rather than a second writer.
>
> *`Redact(events Stream, opts ViewOptions) Stream`*, not a redacting writer beside `WriteSSE`. `goodall.Event` is a sealed interface, so the chat layer cannot mint a redacted event type; every event it yields is one of goodall's own with fields zeroed, which also means the two wire writers, an NDJSON log and a test all take the redacted stream the same way. It withholds what the view withholds: a tool call's `Input` (whole on `ToolCallStart` and `Done.Result.Pending`, and in the `ToolInputDelta` fragments, which are dropped outright since a fragment is nothing but input), a tool result's `Content` and cache marker, the `Message` on any `Response`, and `Result.Conversation`. What a front end is left with is the call's id and name, `IsError` — the same typed status `ToolResultStatus` carries in the view — the tokens, the money and the stop reason. `Stopped` keeps `Cause`, `Message` and `Kind`: that prose is the loop's own account of why the run ended, not content it read.
>
> *Thinking follows the agent's display setting, as the view does:* under `DisplayOmitted` the `ThinkingDelta` still arrives, carrying no text, because the block must still open and close.
>
> *`Service.ViewOptions()` is how a caller outside the package gets the two facts.* `Service.View` computed them from its private agent; a caller redacting a stream needed the same two and had no way to ask. `View` now calls the accessor, so the agent is read in one place, and the system prompt's text still has no way out.
  ...
  - title: goodall v1
    ...
    children:
      - title: Foundation
        ...
        children:
          - title: Scaffold, license and dependency-direction test
            ref: scaffold
            desc: |
          ...
          - title: HTTP transport helper with retries
            ...
            desc: |
              ...
              RateLimited/Overloaded/Server kinds honoring Retry-After, no retry after the first streamed
            ...
            criteria:
              ...
              - Canceling the context mid-body returns ctx.Err and closes the body
      ...
      - title: OpenRouter provider
        ...
        children:
          - title: OpenRouter wire structs, dialects and translation
            ...
            criteria:
              - reasoning_details from a recorded response re-serializes byte-identical and in order on the next request
        ...
        children:
          - title: Loop core, budget and termination
            ...
            criteria:
              ...
              - Canceling mid-stream yields Stopped with the partial assistant message in the conversation
        ...
        children:
          ...
          - title: README and doc audit
            ...
            desc: |
              ...
              consumer and one for the customizer, links to the project docs, authorship and license;
