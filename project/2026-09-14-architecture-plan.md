...
> **Added 2026-09-15 while wiring the catalogue into the loop.** The cache is not the agent's, and the facts travel on the request.
>
> *`Request.ModelInfo *ModelInfo` is the seam.* The loop fetches the entry once per run, before the first turn, and puts it on every turn's Request; a direct consumer may fill it in by hand, and nil means "nobody consulted the catalogue", never "the model can do nothing". Translation therefore stays a pure function of the Request and a provider's `Stream` never makes a hidden catalogue call. Anthropic's thinking translation reads `Capabilities.ThinkingStyle` when it is adaptive or budget and falls back to the model-name heuristic otherwise — with the always-on reading of that heuristic still applied on top of a catalogue "adaptive", since the catalogue says which *form* a model takes and never that it refuses `disabled`.
>
> *The memo lives on the provider clients, not on `Agent`.* `anthropic.Client` and `openrouter.Client` memoise `Model` per identifier under a mutex, successes only; OpenRouter's single catalogue fetch is stored entry by entry, under both id and canonical slug, so the first lookup answers for every model it lists. `Agent` is a plain struct that `chat.NewService` copies by value, so a mutex or an atomic on it would be a `vet` copylocks failure and a split cache. `ModelLister`'s contract says so: the agent calls `Model` at the start of every run, an implementation should memoise, and the built-in providers do. The price is that the first run in a process pays one catalogue round trip before its first token — on OpenRouter the whole catalogue, a few hundred kilobytes — which a short-lived CLI did not pay before; if that ever bites, the fix is a pre-warm on the client, not a change to the loop.
>
> *The check runs every turn; the fetch runs once.* A tool result can carry an image the caller's first message did not. It requires `CapImageInput` for image blocks, `CapPDFInput` for `application/pdf` documents, `CapTools` for a non-empty tool list, `CapThinking` for an effort other than default or off, and `CapCacheControl` for a caller's marker under `CacheManual`. Tool-use and tool-result blocks in the history ask for nothing of their own: a request replaying tool history always carries the definitions, and charging the blocks would make an image inside a tool result report "tools". Only `Unsupported` refuses (invariant 11); a catalogue error, `KindNotFound` included, is logged at Warn and the run proceeds with no facts, because the catalogue can lag the API — there is no negative caching in v1. A provider that does not implement `ModelLister` gets no pre-flight at all.
>
> *`Capabilities.MaxOutput` does not override `max_tokens`.* Anthropic refuses a non-streaming request whose `max_tokens` implies more than ten minutes of generation, so sending a model's published 64k or 128k ceiling would break `Complete` on exactly the models that publish the largest one. `anthropic.DefaultMaxTokens` (8192) stands, and the doc comments on `Agent.MaxTokens`, `Request.MaxTokens` and `DefaultMaxTokens` — which promised "the model's maximum" and "a catalogue will override it" — are corrected to say that the provider substitutes its own named default.
