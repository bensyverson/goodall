...
> **Observed 2026-09-17 by the adapters leaf (Tfq6cs)**, which built both compositions as `typesafe.Tool`, `typesafe.AuthoredTool` and `typesafe.Route`. Three things the sketch above did not say:
>
> 1. **Both compositions held: no core change was needed.** A judgment tool is a `goodall.NewTool` over a one-member input schema, and `Route` is an ordinary `Hooks.BeforeSend` value. The only seam that pinched is `Route`'s: a `Hooks` field is a func, so a mistake in the route's own definition — a nil client, an unaskable question set, a nil callback — has nowhere to be returned at construction and is reported by the first run instead.
> 2. **A router's default has to be fail-closed.** Carrying on unrouted after a failed judgment is invisible: every turn goes to whichever model the agent was built with and nothing says the routing stopped. `Route` ends the run with `StopCauseHook` and takes `WithFallthrough()` for the consumers whose routing is an optimization.
> 3. **The model-authored questions were good, and they lean on the one thing the jaggedness page warns about.** Given five support tickets as a JSON array and no advice beyond the tool's own description, `claude-sonnet-5` asked all ten questions in one call — a choice and a noul per ticket — rather than one call per ticket, which is the habit the description asks for. But it addressed each question to a record *by reference* ("For ticket T-1041 (Payouts failing since Tuesday), which team should handle it?") over the whole array as state, which is exactly the indirection `model-jaggedness/jev-1.13.md` says degrades the model. `jev-1.13.0` resolved all ten anyway, at confidence 1.000 on every choice and with sensible nouls (0.90 for the blocked payouts, 0.05 for the invoice the sender called no-rush). Reproduce with `GOODALL_ENV_FILE=/path/.env go test ./typesafe/ -run TestLiveAuthoredTool -v`. One run in three also refused its first call and succeeded on its second, which is the refusal path working as designed; the refusal's own text was not captured.
...
- **Several specific inspectors beat one vague judge, and the judge makes a good bouncer.** Ask "is this field invented?", "is this off target?", "is this incomplete?" as separate questions and let code combine them, rather than "is this record correct?"; and put the judge between a cheap model and an expensive one, so the expensive one sees only what the judge flagged. Both from a community post the owner passed on, recorded in the [triage example plan](2026-09-17-triage-example-plan.md#community-heuristics-read-2026-09-17), where they changed the draft check.
